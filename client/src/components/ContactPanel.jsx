...
export default function ContactPanel() {
  ...
  const checkContactScroll = useCallback(() => {
    const el = contactItemsRef.current;
    if (!el) return;
    setCanScrollUp(el.scrollTop > 4);
    setCanScrollDown(el.scrollTop + el.clientHeight < el.scrollHeight - 4);
  }, []);
  useLayoutEffect(() => { checkContactScroll(); });
  ...
  async function handleCtxAction(action) {
    ...
    try {
      if (action === 'pin' || action === 'mute') {
        ...
        if (!await useStore.getState().showConfirm(`确认删除好友「${data.display_name}」？删除后对方也将被移出你的好友列表。`)) return;
        ...
        if (isBlocked) {
          await useStore.getState().api(`/users/block/${data.id}`, { method: 'DELETE' });
          useStore.getState().addToast({ title: '已解除拉黑', body: `${data.display_name} 已恢复正常` });
        } else {
          if (!await useStore.getState().showConfirm(`确认拉黑「${data.display_name}」？拉黑后对方仍是好友，但你将不再收到其消息。`)) return;
          await useStore.getState().api(`/users/block/${data.id}`, { method: 'POST' });
          useStore.getState().addToast({ title: '已拉黑', body: `${data.display_name} 已被拉黑` });
        }
        fetchContacts();
      }
  ...
  return (
    <div className="contact-panel">
      ...
      <div className="contact-list">
        ...
        <div className="contact-items" ref={contactItemsRef} onScroll={checkContactScroll}>
          {/* ── Search results ── */}
          {localSearch.trim() && (
        ...
        {/* ↑↓ scroll buttons */}
        <div className="conv-scroll-btns">
          <button
            className={`conv-scroll-btn${canScrollUp ? '' : ' conv-scroll-btn-dim'}`}
            onClick={() => { if (canScrollUp) contactItemsRef.current?.scrollBy({ top: -200, behavior: 'smooth' }); }}
          >
            <svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor"><path d="M7.41 15.41L12 10.83l4.59 4.58L18 14l-6-6-6 6z"/></svg>
          </button>
          <button
            className={`conv-scroll-btn${canScrollDown ? '' : ' conv-scroll-btn-dim'}`}
            onClick={() => { if (canScrollDown) contactItemsRef.current?.scrollBy({ top: 200, behavior: 'smooth' }); }}
          >
        ...
        {showRequests
          ...
          : selected
            ? <ContactDetail
                ...
                onDeleted={() => { setSelected(null); fetchContacts(); fetchConversations(); }}
              />
            : <EmptyDetail />
        }
      </div>
      ...
      {ctxMenu && (
        <div className="conv-ctx-menu" style={{ top: ctxMenu.y || '50%', left: ctxMenu.x || '50%' }}
          ...
          <button onClick={() => handleCtxAction('block')} style={{ color: ctxMenu.data?.is_blocked ? '#07c160' : '#fa9d3b' }}>
            {ctxMenu.data?.is_blocked ? '解除拉黑' : '拉黑'}
          </button>
        ...
        requestAnimationFrame(function scan() {
          ...
          if (v && c && v.readyState === v.HAVE_ENOUGH_DATA) {
            ...
            if (decoded?.data?.startsWith('mixin_code:')) {
              const code = decoded.data.replace('mixin_code:', '').trim();
              if (!/^\d{6}$/.test(code) && !/^\d{8}$/.test(code)) return; // 6位用户码 or 8位群码
              active = false;
              stopStream();
              lookupCode(code);
              return;
...
function ContactDetail({ item, onChat, onClose, onDeleted }) {
  ...
  async function deleteFriend() {
    if (!await useStore.getState().showConfirm(`确认删除好友「${data.display_name}」？`)) return;
    try {
      await api(`/users/friends/${data.id}`, { method: 'DELETE' });
      onDeleted?.();
    } catch (e) { useStore.getState().addToast({ title: '删除失败', body: e.message }); }
  }

  async function toggleBlock() {
    try {
      if (isBlocked) {
        await api(`/users/block/${data.id}`, { method: 'DELETE' });
        setLocalBlocked(false);
        useStore.getState().addToast({ title: '已解除拉黑', body: `${data.display_name} 已恢复正常` });
      } else {
        if (!await useStore.getState().showConfirm(`确认拉黑「${data.display_name}」？`)) return;
        await api(`/users/block/${data.id}`, { method: 'POST' });
        ...
        {!isGroup && !isSelf && isAlreadyContact && (
          <>
            <button className="btn-block" onClick={toggleBlock}
              style={{ background: isBlocked ? '#f0f0f0' : '#fff4e0', color: isBlocked ? '#07c160' : '#fa9d3b',
                       border: `1px solid ${isBlocked ? '#07c160' : '#fa9d3b'}` }}>
              {isBlocked ? '解除拉黑' : '拉黑'}
            </button>
            <button className="btn-delete-friend" onClick={deleteFriend}>
              删除好友
            </button>
          </>
        )}
