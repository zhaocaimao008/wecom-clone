...
export default function CallScreen() {
  ...
  if (activeCall.state === 'active' && minimized) {
    return (
      <>
        <audio ref={remoteAudioRef} autoPlay playsInline style={{ display: 'none' }} />
        <div className="call-mini-bar">
          <div className="call-mini-info" onClick={() => setMinimized(false)}>
            <AvatarCircle name={peerName} color={peerColor} size={32} radius={16} />
            <div className="call-mini-text">
              <span className="call-mini-name">{peerName}</span>
              <span className="call-mini-timer">{fmtDuration(duration)}</span>
            </div>
          </div>
          <div className="call-mini-actions">
            <button className={`call-mini-btn ${muted ? 'active' : ''}`} onClick={toggleMute} title={muted ? '取消静音' : '静音'}>
              {muted
                ...
                : <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M12 14c1.66 0 2.99-1.34 2.99-3L15 5c0-1.66-1.34-3-3-3S9 3.34 9 5v6c0 1.66 1.34 3 3 3zm5.3-3c0 3-2.54 5.1-5.3 5.1S6.7 14 6.7 11H5c0 3.41 2.72 6.23 6 6.72V21h2v-3.28c3.28-.48 6-3.3 6-6.72h-1.7z"/></svg>
              }
            </button>
            <button className="call-mini-btn call-mini-end" onClick={() => endCall(peerId)} title="挂断">
              <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M20.01 15.38c-1.23 0-2.42-.2-3.53-.56-.35-.12-.74-.03-1.01.24l-1.57 1.97c-2.83-1.35-5.48-3.9-6.89-6.83l1.95-1.66c.27-.28.35-.67.24-1.02-.37-1.12-.56-2.3-.56-3.53 0-.54-.45-.99-.99-.99H4.19C3.65 3 3 3.24 3 3.99 3 13.28 10.73 21 20.01 21c.71 0 .99-.63.99-1.18v-3.45c0-.54-.45-.99-.99-.99z"/></svg>
            </button>
          </div>
        </div>
      </>
    ...
    <>
      ...
      <div className={`call-overlay ${isVideo ? 'call-video-mode' : ''}`}>
        <div className="call-box active">
          {/* Minimize button */}
          <button className="call-minimize-btn" onClick={() => setMinimized(true)} title="最小化">
